...
namespace advanced_jobmatchingtool_webapp.Controllers.Klant
{
    ...
    public class BehoeftesKlantController : Controller
    {
        private readonly IAntwoordKlantService _antwoordKlantService;
        private readonly UserManager<ApplicationUser> _userManager;
        private ILogger<BehoeftesKlantController> _logger;
        private readonly ApplicationDbContext _context;

        public BehoeftesKlantController(IAntwoordKlantService antwoordKlantService, UserManager<ApplicationUser> userManager, ILogger<BehoeftesKlantController> logger, ApplicationDbContext context)
        {
            _antwoordKlantService = antwoordKlantService;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var vragen = await _antwoordKlantService.GetVragenByNaamCategorieAsync("Behoeftes");
            var antwoorden = await _context.AntwoordenKlanten
                .Where(a => a.UserId == userId && a.Categorie.StartsWith("Behoeftes"))
                .ToListAsync();

            var model = new AntwoordKlantIndexViewModel
            {
                UserId = userId,
                Voornaam = user.Voornaam,
                Familienaam = user.Familienaam,
                VragenAntwoorden = vragen.Select(v => new VraagAntwoordKlantViewModel
                {
                    VraagId = v.Id,
                    VraagText = v.VraagText,
                    Type = v.SoortAntwoord.ToString(),
                    Opties = v.AntwoordOptie.OptieTekst?.Split(", ").ToList(),
                    Antwoord = antwoorden.FirstOrDefault(a => a.VraagKlantId == v.Id)?.AntwoordTekst,
                    Antwoorden = antwoorden.FirstOrDefault(a => a.VraagKlantId == v.Id)?.AntwoordTekst?.Split(", ").ToList()
                }).ToList(),
        ...
        public async Task<IActionResult> SaveAntwoorden(List<VraagAntwoordKlantViewModel> vragenAntwoorden)
        {
            var userId = _userManager.GetUserId(User);
            foreach (var item in vragenAntwoorden)
            {
                ...
                //opslaan in MySQL
                var bestaandAntwoord = await _context.AntwoordenKlanten
                    .FirstOrDefaultAsync(a => a.UserId == userId && a.VraagKlantId == item.VraagId);
                ...
                {
                    var vraag = await _antwoordKlantService.GetVraagKlantByIdAsync(item.VraagId);
                    if (vraag == null)
                    {
                    ...
                    var nieuwAntwoord = new AntwoordKlant
                    {
                        UserId = userId,
                        VraagKlantId = item.VraagId,
                    ...
                    _context.AntwoordenKlanten.Add(nieuwAntwoord);
            ...
            return RedirectToAction("Index", "BehoeftesKlant");
        }
    }
}

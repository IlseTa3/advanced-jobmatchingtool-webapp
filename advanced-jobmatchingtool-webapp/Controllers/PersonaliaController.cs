...
using advanced_jobmatchingtool_webapp.ViewModels.Personalia;
...
namespace advanced_jobmatchingtool_webapp.Controllers
{
    ...
    public class PersonaliaController : Controller
    {
        ...
        private readonly ILogger<PersonaliaController> _logger;

        public PersonaliaController(MongoDbVragenPerCategorieService vragenPerCategorieService, UserManager<ApplicationUser> userManager, ApplicationDbContext context, ILogger<PersonaliaController> logger)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var vragen = await _vragenPerCategorieService.GetVragenByCategorieAsync("Personalia");

            var antwoorden = await _context.PersonaliaKandidaten
                .Where(a => a.UserId == userId && a.Categorie == "Personalia")
                .ToListAsync();

            var model = new PersonaliaViewModel
            {
                UserId = userId,
                Voornaam = user.Voornaam,
                Familienaam = user.Familienaam,
                VragenAntwoorden = vragen.Select(v => new VraagAntwoordPersonaliaViewModel
                {
                    VraagId = v.Id,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveAntwoorden(List<VraagAntwoordPersonaliaViewModel> vragenAntwoorden)
        {
            var userId = _userManager.GetUserId(User);
            ...
            foreach (var item in vragenAntwoorden)
            {
                ...
                // Opslaan in MySQL
                var bestaandAntwoord = await _context.PersonaliaKandidaten
                ...
                else
                {
                    ...
                    var nieuwAntwoord = new PersonaliaKandidaat
                    {
                        UserId = userId,
                    ...
                    _context.PersonaliaKandidaten.Add(nieuwAntwoord);
            ...
            return RedirectToAction("Index", "Personalia");
        }
    }
}

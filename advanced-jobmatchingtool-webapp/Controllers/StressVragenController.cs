...
using advanced_jobmatchingtool_webapp.ViewModels.Stress;
...
namespace advanced_jobmatchingtool_webapp.Controllers
{
    ...
    public class StressVragenController : Controller
    {
        private readonly MongoDbVragenPerCategorieService _vragenPerCategorieService;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly ApplicationDbContext _context;
        private readonly ILogger<StressVragenController> _logger;

        public StressVragenController(MongoDbVragenPerCategorieService vragenPerCategorieService, UserManager<ApplicationUser> userManager, ApplicationDbContext context, ILogger<StressVragenController> logger)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var vragen = await _vragenPerCategorieService.GetVragenByClusteredCategories("^Stress");
            var antwoorden = await _context.StressKandidaten
                .Where(a => a.UserId == userId && a.Categorie.StartsWith("Stress"))
                .ToListAsync();


            var model = new StressViewModel
            {
                UserId = userId,
                Voornaam = user.Voornaam,
                Familienaam = user.Familienaam,
                VragenAntwoorden = vragen.Select(v => new VraagAntwoordStressViewModel
                {
                    VraagId = v.Id,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveAntwoorden(List<VraagAntwoordStressViewModel> vragenAntwoorden)
        {
            var userId = _userManager.GetUserId(User);
            ...
            foreach (var item in vragenAntwoorden)
            {
                ...
                // Opslaan in MySQL
                var bestaandAntwoord = await _context.StressKandidaten
                ...
                {
                    var vraag = await _vragenPerCategorieService.GetVraagByIdAsync(item.VraagId);
                    // Nieuw antwoord toevoegen
                    var nieuwAntwoord = new StressKandidaat
                    {
                        UserId = userId,
                    ...
                    _context.StressKandidaten.Add(nieuwAntwoord);
            ...
            return RedirectToAction("Index", "StressVragen");
        }
    }
}

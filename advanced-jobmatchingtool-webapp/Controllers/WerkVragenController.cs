...
using advanced_jobmatchingtool_webapp.ViewModels.Werk;
...
namespace advanced_jobmatchingtool_webapp.Controllers
{
    ...
    public class WerkVragenController : Controller
    {
        private readonly MongoDbVragenPerCategorieService _vragenPerCategorieService;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly ApplicationDbContext _context;
        private readonly ILogger<WerkVragenController> _logger;

        public WerkVragenController(MongoDbVragenPerCategorieService vragenPerCategorieService, UserManager<ApplicationUser> userManager, ApplicationDbContext context, ILogger<WerkVragenController> logger)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var vragen = await _vragenPerCategorieService.GetVragenByClusteredCategories("^Werk");

            var antwoorden = await _context.WerkKandidaten
                .Where(a => a.UserId == userId && a.Categorie.StartsWith("Werk"))
                .ToListAsync();

            var model = new WerkViewModel
            {
                UserId = userId,
                Voornaam = user.Voornaam,
                Familienaam = user.Familienaam,
                VragenAntwoorden = vragen.Select(v => new VraagAntwoordWerkViewModel
                {
                    VraagId = v.Id,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveAntwoorden(List<VraagAntwoordWerkViewModel> vragenAntwoorden)
        {
            var userId = _userManager.GetUserId(User);
            ...
            foreach (var item in vragenAntwoorden)
            {
                ...
                // Opslaan in MySQL
                var bestaandAntwoord = await _context.WerkKandidaten
                ...
                else
                {
                    ...
                    var nieuwAntwoord = new WerkKandidaat
                    {
                        UserId = userId,
                    ...
                    _context.WerkKandidaten.Add(nieuwAntwoord);
            ...
            return RedirectToAction("Index", "WerkVragen");
        }
    }
}
